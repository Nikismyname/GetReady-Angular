...
export class ReorderService {
    ...
    generateOrderingWithElements(questions: IQGlobalIndex[], currInd: number, prevIndex: number, currCol: number, prevCol: number) {
        ...
        let initialOrders = [];
        for (let i = 0; i < questionCount; i++) {
            initialOrders.push([questions[i].id, i]);
        }
    ...
    generateReorderingsSheet(qSheets: any[], currInd: number, prevInd: number) {
        let array: number[][] = []; 
        for (let i = 0; i < qSheets.length; i++) {      
            array.push([qSheets[i].id, i]);
        }
