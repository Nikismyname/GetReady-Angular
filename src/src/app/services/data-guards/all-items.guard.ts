...
export class AllItemsGuard implements CanActivate {
    constructor(
        private store: Store<IAppState>,
    ) { }

    fetchStream() {
        return this.store.select(x => x.crud.read.allItems).pipe(
            filter(x => x.success), 
            switchMap(x => {
    ...
    canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
        ...
        let isGlobal = next.data["items"] as boolean

        this.store.dispatch(new ReadActions.GetAllItems(isGlobal));
